...
class OrdersContact extends \common\models\BaseModel
{
    ...
    public function checkBillEmpty($attribute, $params, $validator)
    {
        if ($this->$attribute == self::STATIC_PAYMENT_TRANSFER && empty($this->bill_link)) {
            $this->addError($attribute, 'Hóa đơn rỗng!');
        }
    }
    ...
    public function beforeSave($insert)
    {
        ...
        if (!$this->bill_link && $this->payment_method == static::STATIC_PAYMENT_TRANSFER) {
            $this->addError('bill_link', 'Hoá đơn rỗng!');
            return false;
        }
